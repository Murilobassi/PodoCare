...
function refreshAdmin() {
  ...
  const filtered = allAppointments.filter(a => {
    ...
    const inRange = period === 'hoje' ? d === today : (d >= start && d <= today);
    return inRange && (prof === 'todos' || a.podologa_name === prof);
  });
  ...
  document.getElementById('admin-revenue').innerHTML =
    '<span style="font-size:16px;color:var(--muted);font-weight:400;font-family:\'DM Sans\',sans-serif">R$&nbsp;</span>' + fmtPrice(revenue);
  document.getElementById('admin-ticket').innerHTML =
    '<span style="font-size:16px;color:var(--muted);font-weight:400;font-family:\'DM Sans\',sans-serif">R$&nbsp;</span>' + fmtPrice(ticket);
...
function renderUpcoming(prof, today) {
  const upcoming = allAppointments.filter(a => {
    ...
    return futuro && (prof === 'todos' || a.podologa_name === prof);
  });
...
function renderPastList(prof, today) {
  const past = allAppointments.filter(a => {
    ...
    return passou && (prof === 'todos' || a.podologa_name === prof);
  }).slice(0, 6);
...
function renderFullAgenda() {
  ...
  if (prof   !== 'todos') list = list.filter(a => a.podologa_name === prof);
