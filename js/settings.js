...
async function loadSettings() {
  ...
  const [{ data: profs }, { data: services }, { data: profiles }] = await Promise.all([
...
function renderProfs(list) {
  ...
  container.innerHTML = '';
  list.forEach(b => {
    const row  = document.createElement('div'); row.className = 'item-row animate-item';
...
function renderAccess(list) {
  ...
  if (list.length === 0) {
    container.innerHTML = '<div class="empty-state" style="padding:20px"><p>Nenhum perfil encontrado</p></div>';
    return;
  }
  ...
  list.forEach(p => {
    const row  = document.createElement('div'); row.className = 'item-row animate-item';
    const info = document.createElement('div');
    const nm   = document.createElement('div'); nm.className = 'item-name'; nm.textContent = p.full_name || p.email || 'Usuário Sem Nome';
    const mt   = document.createElement('div'); mt.className = 'item-meta'; mt.textContent = p.email || '';
    info.append(nm, mt);

    const right = document.createElement('div'); 
    right.style.display = 'flex'; right.style.gap = '8px'; right.style.alignItems = 'center';

    // Para o MASTER original, não deixa alterar o próprio cargo
    if (p.role === 'MASTER') {
      ...
      sel.className = 'add-input';
      sel.style.padding = '4px 8px'; sel.style.fontSize = '12px'; sel.style.width = 'auto';

      const optPodo = document.createElement('option'); optPodo.value = 'PODOLOGA'; optPodo.textContent = 'Podóloga';
      const optAdmin = document.createElement('option'); optAdmin.value = 'ADMIN'; optAdmin.textContent = 'Admin';
      
      sel.appendChild(optPodo);
      sel.appendChild(optAdmin);
      sel.value = p.role;

      sel.addEventListener('change', () => updateUserRole(p.id, sel.value, p.full_name || p.email));
...
async function updateUserRole(id, newRole, name) {
  if (currentRole !== 'MASTER') return;
  const { error } = await sb.rpc('set_user_role', { target_id: id, new_role: newRole });
  if (error) {
    toast('Ação bloqueada ou falhou. O Supabase não permitiu a alteração.', 'error');
    // Força voltar pro valor que no DB (recarregar página ou dado original)
