...
async function loadPodologaData(nomeCompleto) {
  ...
  const firstName = rawName.replace(/@.*/, '').split(' ')[0].trim(); // Tira o provedor de e-mail e pega o primeiro nome

  const [{ data: apts }, { data: svcs }] = await Promise.all([
    // Usa ilike com % para buscar de forma super elástica pelo primeiro nome ("Isabel" vai dar match)
    sb.from('atendimentos').select('*').ilike('podologa_name', `%${firstName}%`).order('date', { ascending: false }),
