...
function renderHistory(appointments, today) {
  ...
  history.forEach(a => {
    ...
    const pr    = document.createElement('div'); pr.className = 'past-price'; pr.style.color = 'var(--gold)'; pr.textContent = 'R$ ' + fmtPrice(a.price);
    const dt    = document.createElement('div');
    dt.style.cssText = 'font-size:10px;color:var(--muted);margin-top:4px;text-transform:uppercase;letter-spacing:0.05em;font-weight:700';
...
function refreshPodologaFin() {
  ...
  document.getElementById('fin-total-val').innerHTML =
    '<span style="font-size:24px;color:rgba(255,255,255,0.25);font-weight:300;font-family:var(--font-serif);font-style:italic">R$&nbsp;</span>' + fmtPrice(totalAll);
  document.getElementById('fin-total-val').style.color = '#fff';
