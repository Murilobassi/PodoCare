...
async function loadPodologaData(nomeCompleto) {
  ...
  debugStr = `Banco (${allApts.length}) | Busca (${firstName})`;
...
function refreshPodologaFin() {
  ...
  document.getElementById('fin-total-desc').textContent = `${totalCount} atendimento(s) | Debug: [${debugStr} | Filtro: ${filtered.length}]`;
}
