...
async function loadPodologaData(nomeCompleto) {
  // Usa o nome vindo do banco e remove espaços laterais para evitar bugs do banco
  const rawName = nomeCompleto || currentUser?.user_metadata?.full_name || currentUser?.email || '';
  const name = rawName.split('@')[0].trim(); // Pega apenas a primeira parte se for email

  const [{ data: apts }, { data: svcs }] = await Promise.all([
    // Usa ilike com % para ser case-insensitive e tolerar espaços soltos
    sb.from('atendimentos').select('*').ilike('podologa_name', `%${name}%`).order('date', { ascending: false }),
    sb.from('servicos').select('*').order('name'),
  ]);

  const today        = todayStr();
  podoAppointments   = apts || [];
  const services     = svcs || [];
