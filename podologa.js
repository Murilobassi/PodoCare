...
async function loadPodologaData() {
  // Usa o nome completo da podóloga como chave de busca
...
function renderServices(services) {
  ...
  services.forEach(s => {
    ...
    // Botão de deletar: ID guardado em closure, sem interpolação em onclick
...
function toggleAddSvc() {
  const f = document.getElementById('add-svc-form');
  f.style.display = f.style.display === 'none' ? 'block' : 'none';
}

async function addService() {
  const name  = document.getElementById('svc-name').value.trim();
  const dur   = document.getElementById('svc-duration').value;
...
async function deleteService(id) {
  showConfirmModal('Remover Serviço?', 'Essa ação não pode ser desfeita.', async () => {
